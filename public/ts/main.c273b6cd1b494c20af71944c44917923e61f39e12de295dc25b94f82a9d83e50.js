...
Category: ${t.category}`),i}updateProgress(e,t,i){this.progressCallback&&this.progressCallback({stage:e,progress:t,message:i}),this.config.utils.debugLog(`Upload Progress: ${e} - ${t}% - ${i}`)}delay(e){return new Promise(t=>setTimeout(t,e))}async getUploadStats(){return{totalUploads:0,totalSize:0,categories:{},recentUploads:[]}}async deleteImage(e){if(!this.token)throw new Error(this.config.utils.getErrorMessage("auth_failed"));try{let{owner:t,repo:i,branch:s}=this.config.imageRepo,a=`${this.config.apiUrl}/repos/${t}/${i}/contents/${e}`,o=await fetch(a,{headers:{Authorization:`token ${this.token}`,Accept:"application/vnd.github.v3+json"}});if(!o.ok)throw new Error("File not found");let r=await o.json(),c=`${this.config.apiUrl}/repos/${t}/${i}/contents/${e}`;return(await fetch(c,{method:"DELETE",headers:{Authorization:`token ${this.token}`,"Content-Type":"application/json",Accept:"application/vnd.github.v3+json"},body:JSON.stringify({message:`Delete image: ${e}`,sha:r.sha,branch:s})})).ok}catch(t){return this.config.utils.debugLog("Delete failed:",t),!1}}async listImages(e){if(!this.token)throw new Error(this.config.utils.getErrorMessage("auth_failed"));try{let{owner:t,repo:i}=this.config.imageRepo,s=e?this.config.imageUpload.paths[e]||e:"",a=`${this.config.apiUrl}/repos/${t}/${i}/contents/${s}`,o=await fetch(a,{headers:{Authorization:`token ${this.token}`,Accept:"application/vnd.github.v3+json"}});if(!o.ok)throw new Error("Failed to list images");return(await o.json()).filter(c=>{let u=c.name.split(".").pop()?.toLowerCase();return this.config.imageUpload.supportedFormats.includes(u)})}catch(t){return this.config.utils.debugLog("List images failed:",t),[]}}},C=class{uploader;container=null;constructor(e){this.uploader=e}openManager(){this.container||this.createInterface(),this.container&&(this.container.style.display="block")}closeManager(){this.container&&(this.container.style.display="none")}createInterface(){document.body.insertAdjacentHTML("beforeend",`
        ...
        `,n.setAttribute("aria-label","\u8FD4\u56DE\u9876\u90E8"),document.body.appendChild(n);let e=!1,t=()=>{(window.pageYOffset||document.documentElement.scrollTop)>300?n.classList.add("visible"):n.classList.remove("visible"),e=!1};window.addEventListener("scroll",()=>{e||(requestAnimationFrame(t),e=!0)}),n.addEventListener("click",i=>{i.preventDefault(),window.scrollTo({top:0,behavior:"smooth"})}),console.log("\u2705 Back to top button setup complete")},setupSmoothScrolling(){document.querySelectorAll('a[href^="#"]').forEach(e=>{e.addEventListener("click",t=>{let i=e.getAttribute("href");if(i==="#"||i==="#top"){t.preventDefault(),window.scrollTo({top:0,behavior:"smooth"});return}let s=document.querySelector(i);if(s){t.preventDefault();let a=s.getBoundingClientRect().top+window.pageYOffset-80;window.scrollTo({top:a,behavior:"smooth"})}})}),console.log("\u2705 Smooth scrolling setup complete")},setupImageLazyLoading(){if("IntersectionObserver"in window){let n=new IntersectionObserver((t,i)=>{t.forEach(s=>{if(s.isIntersecting){let a=s.target;a.style.opacity="0",a.style.transition="opacity 0.3s ease",a.addEventListener("load",()=>{a.style.opacity="1"}),i.unobserve(a)}})});document.querySelectorAll(".article-image img").forEach(t=>{n.observe(t)}),console.log("\u2705 Enhanced image lazy loading setup complete")}},setupAnimations(){if("IntersectionObserver"in window){let n=new IntersectionObserver(i=>{i.forEach(s=>{s.isIntersecting&&s.target.classList.add("animate-in")})},{threshold:.1,rootMargin:"0px 0px -50px 0px"});document.querySelectorAll(".article-list article").forEach((i,s)=>{i.style.animationDelay=`${s*.1}s`,n.observe(i)}),document.querySelectorAll(".widget").forEach((i,s)=>{i.style.animationDelay=`${s*.1}s`,n.observe(i)}),console.log("\u2705 Scroll animations setup complete")}this.setupHoverEffects()},setupHoverEffects(){document.querySelectorAll(".article-list article").forEach(t=>{t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-8px) rotateX(2deg)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0) rotateX(0deg)"})}),document.querySelectorAll(".tag-cloud a").forEach(t=>{t.addEventListener("mouseenter",()=>{let i=["linear-gradient(135deg, #667eea 0%, #764ba2 100%)","linear-gradient(135deg, #f093fb 0%, #f5576c 100%)","linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)","linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)","linear-gradient(135deg, #fa709a 0%, #fee140 100%)"],s=i[Math.floor(Math.random()*i.length)];t.style.background=s})}),console.log("\u2705 Hover effects setup complete")}};ee.init();window.FrontendBeautify=ee;var te={init(){console.log("\u{1F9ED} Initializing navigation enhancement..."),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{this.setupAll()}):this.setupAll()},setupAll(){this.setupScrollEffects(),this.setupScrollIndicator(),this.setupMobileMenu(),this.setupSearchEnhancement(),console.log("\u2705 Navigation enhancement initialized")},setupScrollEffects(){let n=document.querySelector(".main-header");if(!n)return;let e=window.scrollY,t=!1,i=()=>{let s=window.scrollY;s>50?n.classList.add("scrolled"):n.classList.remove("scrolled"),s>e&&s>200?n.style.transform="translateY(-100%)":n.style.transform="translateY(0)",e=s,t=!1};window.addEventListener("scroll",()=>{t||(requestAnimationFrame(i),t=!0)}),console.log("\u2705 Scroll effects setup complete")},setupScrollIndicator(){let n=document.createElement("div");n.className="scroll-indicator",n.innerHTML='<div class="scroll-indicator-bar"></div>',document.body.appendChild(n);let e=n.querySelector(".scroll-indicator-bar"),t=!1,i=()=>{let s=window.pageYOffset||document.documentElement.scrollTop,a=document.documentElement.scrollHeight-window.innerHeight,o=s/a*100;e&&(e.style.width=`${Math.min(o,100)}%`),t=!1};window.addEventListener("scroll",()=>{t||(requestAnimationFrame(i),t=!0)}),console.log("\u2705 Scroll indicator setup complete")},setupMobileMenu(){let n=document.querySelector(".menu-toggle"),e=document.querySelector(".main-menu");if(!n||!e)return;n.addEventListener("click",()=>{n.classList.toggle("active"),e.classList.toggle("active"),document.body.classList.toggle("menu-open")}),e.querySelectorAll("a").forEach(i=>{i.addEventListener("click",()=>{n.classList.remove("active"),e.classList.remove("active"),document.body.classList.remove("menu-open")})}),document.addEventListener("click",i=>{let s=i.target;!n.contains(s)&&!e.contains(s)&&(n.classList.remove("active"),e.classList.remove("active"),document.body.classList.remove("menu-open"))}),console.log("\u2705 Mobile menu enhancement setup complete")},setupSearchEnhancement(){let n=document.querySelector(".search-button"),e=document.querySelector(".search-modal");if(n){if(n.addEventListener("click",t=>{if(t.preventDefault(),n.style.transform="scale(0.95)",setTimeout(()=>{n.style.transform=""},150),e){e.classList.add("active");let i=e.querySelector("input");i&&setTimeout(()=>i.focus(),100)}}),e){let t=e.querySelector(".search-close");t&&t.addEventListener("click",()=>{e.classList.remove("active")}),document.addEventListener("keydown",i=>{i.key==="Escape"&&e.classList.contains("active")&&e.classList.remove("active")}),e.addEventListener("click",i=>{i.target===e&&e.classList.remove("active")})}console.log("\u2705 Search enhancement setup complete")}}};te.init();window.NavigationEnhance=te;var ie={init(){console.log("\u{1F3A8} Initializing admin panel enhancement..."),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{this.setupAll()}):this.setupAll()},setupAll(){this.setupAnimations(),this.setupFormEnhancements(),this.setupNotifications(),this.setupKeyboardShortcuts(),this.setupTooltips(),this.setupProgressIndicators(),console.log("\u2705 Admin panel enhancement initialized")},setupAnimations(){let n=()=>{if("IntersectionObserver"in window){let t=new IntersectionObserver(s=>{s.forEach(a=>{a.isIntersecting&&a.target.classList.add("animate-in")})},{threshold:.1,rootMargin:"0px 0px -50px 0px"});document.querySelectorAll(".admin-tab-panel, .admin-section, .admin-action-btn").forEach((s,a)=>{s.style.animationDelay=`${a*.1}s`,t.observe(s)})}},e=document.getElementById("admin-panel-modal");e&&new MutationObserver(i=>{i.forEach(s=>{s.type==="attributes"&&s.attributeName==="style"&&s.target.style.display==="flex"&&setTimeout(n,100)})}).observe(e,{attributes:!0,attributeFilter:["style"]}),console.log("\u2705 Admin panel animations setup complete")},setupFormEnhancements(){let n=()=>{document.querySelectorAll(".admin-form-group").forEach(s=>{let a=s.querySelector("input, textarea"),o=s.querySelector("label");if(a&&o&&!s.classList.contains("floating-label")){s.classList.add("floating-label");let r=()=>{a.value||a===document.activeElement?s.classList.add("floating"):s.classList.remove("floating")};a.addEventListener("focus",r),a.addEventListener("blur",r),a.addEventListener("input",r),r()}})},e=()=>{document.querySelectorAll(".admin-form-group input, .admin-form-group textarea").forEach(s=>{s.addEventListener("input",a=>{let o=a.target,r=o.closest(".admin-form-group");r&&(r.classList.remove("valid","invalid"),o.value.length>0&&(o.checkValidity()?r.classList.add("valid"):r.classList.add("invalid")))})})},t=()=>{document.querySelectorAll(".admin-btn").forEach(s=>{s.addEventListener("click",()=>{s.classList.contains("loading")||(s.classList.add("loading"),setTimeout(()=>{s.classList.remove("loading")},2e3))})})};n(),e(),t(),console.log("\u2705 Form enhancements setup complete")},setupNotifications(){let n=document.getElementById("admin-notifications");n||(n=document.createElement("div"),n.id="admin-notifications",n.style.cssText=`
            ...
            `,document.body.appendChild(n));let e=s=>{let a=s.getAttribute("title")||s.getAttribute("data-tooltip");a&&(s.removeAttribute("title"),s.setAttribute("data-tooltip",a),s.addEventListener("mouseenter",o=>{let r=o.target,c=r.getBoundingClientRect(),u=r.getAttribute("data-tooltip");u&&n&&(n.textContent=u,n.style.left=`${c.left+c.width/2}px`,n.style.top=`${c.top-10}px`,n.style.transform="translateX(-50%) translateY(-100%)",n.style.opacity="1")}),s.addEventListener("mouseleave",()=>{n&&(n.style.opacity="0")}))};document.querySelectorAll("[title], [data-tooltip]").forEach(e),new MutationObserver(s=>{s.forEach(a=>{a.addedNodes.forEach(o=>{if(o.nodeType===Node.ELEMENT_NODE){let r=o;(r.hasAttribute("title")||r.hasAttribute("data-tooltip"))&&e(r),r.querySelectorAll("[title], [data-tooltip]").forEach(e)}})})}).observe(document.body,{childList:!0,subtree:!0}),console.log("\u2705 Tooltips setup complete")},setupProgressIndicators(){let n=(t,i=0)=>{let s=t.querySelector(".admin-progress-bar");if(!s){s=document.createElement("div"),s.className="admin-progress-bar",s.style.cssText=`
                ...
                `;let o=document.createElement("div");o.className="admin-progress-fill",o.style.cssText=`
                    ...
                    border-radius: 2px;
                `,s.appendChild(o),t.appendChild(s)}let a=s.querySelector(".admin-progress-fill");return a&&(a.style.width=`${Math.min(Math.max(i,0),100)}%`),s};document.querySelectorAll(".admin-form").forEach(t=>{t.addEventListener("submit",i=>{let s=i.target,a=n(s),o=0,r=setInterval(()=>{o+=Math.random()*20;let c=a.querySelector(".admin-progress-fill");c&&(c.style.width=`${Math.min(o,90)}%`),o>=90&&clearInterval(r)},200);setTimeout(()=>{clearInterval(r);let c=a.querySelector(".admin-progress-fill");c&&(c.style.width="100%"),setTimeout(()=>{a.parentNode&&a.parentNode.removeChild(a)},500)},3e3)})}),window.createAdminProgressBar=n,console.log("\u2705 Progress indicators setup complete")}};ie.init();window.AdminPanelEnhance=ie;var B=class{shareData;qrCodeGenerated=!1;constructor(){this.shareData=window.shareData||{url:window.location.href,title:document.title,description:"",image:""},this.init()}init(){console.log("\u{1F517} \u521D\u59CB\u5316\u793E\u4EA4\u5206\u4EAB\u529F\u80FD..."),this.bindEvents(),console.log("\u2705 \u793E\u4EA4\u5206\u4EAB\u529F\u80FD\u521D\u59CB\u5316\u5B8C\u6210")}bindEvents(){let e=document.querySelector(".share-btn--wechat");e&&e.addEventListener("click",()=>this.handleWechatShare());let t=document.querySelector(".share-btn--copy");t&&t.addEventListener("click",()=>this.copyToClipboard());let i=document.getElementById("wechat-qr-close");i&&i.addEventListener("click",()=>this.closeWechatModal());let s=document.getElementById("wechat-qr-modal");s&&s.addEventListener("click",a=>{a.target===s&&this.closeWechatModal()}),document.addEventListener("keydown",a=>{a.key==="Escape"&&this.closeWechatModal()}),console.log("\u2705 \u5206\u4EAB\u4E8B\u4EF6\u76D1\u542C\u5668\u7ED1\u5B9A\u5B8C\u6210")}handleWechatShare(){console.log("\u{1F4F1} \u6253\u5F00\u5FAE\u4FE1\u5206\u4EAB\u4E8C\u7EF4\u7801..."),this.showWechatModal(),this.qrCodeGenerated||this.generateQRCode()}showWechatModal(){let e=document.getElementById("wechat-qr-modal");e&&(e.classList.add("show"),document.body.style.overflow="hidden")}closeWechatModal(){let e=document.getElementById("wechat-qr-modal");e&&(e.classList.remove("show"),document.body.style.overflow="")}generateQRCode(){let e=document.getElementById("wechat-qr-code");if(!e)return;let t=`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(this.shareData.url)}`,i=document.createElement("img");i.src=t,i.alt="\u5FAE\u4FE1\u5206\u4EAB\u4E8C\u7EF4\u7801",i.style.width="200px",i.style.height="200px",e.innerHTML="",e.appendChild(i),this.qrCodeGenerated=!0,console.log("\u2705 \u4E8C\u7EF4\u7801\u751F\u6210\u5B8C\u6210")}async copyToClipboard(){try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(this.shareData.url);else{let e=document.createElement("textarea");e.value=this.shareData.url,e.style.position="fixed",e.style.left="-999999px",e.style.top="-999999px",document.body.appendChild(e),e.focus(),e.select(),document.execCommand("copy"),e.remove()}this.showCopySuccess(),console.log("\u2705 \u94FE\u63A5\u5DF2\u590D\u5236\u5230\u526A\u8D34\u677F")}catch(e){console.error("\u274C \u590D\u5236\u5931\u8D25:",e),this.showCopyError()}}showCopySuccess(){this.showNotification("\u94FE\u63A5\u5DF2\u590D\u5236\u5230\u526A\u8D34\u677F\uFF01","success")}showCopyError(){this.showNotification("\u590D\u5236\u5931\u8D25\uFF0C\u8BF7\u624B\u52A8\u590D\u5236\u94FE\u63A5","error")}showNotification(e,t="success"){let i=document.createElement("div");i.className=`share-notification share-notification--${t}`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>{i.classList.add("show")},100),setTimeout(()=>{i.classList.remove("show"),setTimeout(()=>{i.parentNode&&i.parentNode.removeChild(i)},300)},3e3)}getShareData(){return this.shareData}};document.addEventListener("DOMContentLoaded",()=>{document.querySelector(".article-share")&&new B});window.SocialShare=B;var p,ne=!1,l={init:()=>{if(ne){console.log("\u2139\uFE0F Stack already initialized, skipping...");return}if(ne=!0,p)console.log("\u2139\uFE0F globalAuth already exists, skipping creation");else try{p=new J,console.log("\u2705 globalAuth created successfully")}catch(r){console.error("\u274C Failed to create globalAuth:",r),console.log("\u{1F527} Attempting fallback auth creation..."),p={config:{adminPassword:localStorage.getItem("adminPassword")||"admit"},isAuthenticated:()=>localStorage.getItem("adminAuth")==="authenticated",authenticate:function(c){return c===this.config.adminPassword?(localStorage.setItem("adminAuth","authenticated"),setTimeout(()=>{document.querySelectorAll("[data-admin-only]").forEach(m=>{m.style.display="block"}),document.querySelectorAll("[data-guest-only]").forEach(m=>{m.style.display="none"}),console.log("\u2705 Fallback auth UI updated")},100),!0):!1},logout:()=>{localStorage.removeItem("adminAuth"),document.querySelectorAll("[data-admin-only]").forEach(d=>{d.style.display="none"}),document.querySelectorAll("[data-guest-only]").forEach(d=>{d.style.display="block"})},updatePassword:function(c){this.config.adminPassword=c,localStorage.setItem("adminPassword",c),console.log("\u2705 Fallback auth password updated")}},console.log("\u2705 Fallback auth created")}console.log("Creating login modal...");let n=v.createLoginModal();if(document.body.insertAdjacentHTML("beforeend",n),console.log("Login modal created"),console.log("Checking for admin panel in HTML..."),document.getElementById("admin-panel-modal")?console.log("\u2705 Admin panel found in HTML template"):console.error("\u274C Admin panel not found in HTML template!"),l.bindAuthEvents(),l.bindAdminPanelEvents(),console.log("\u{1F50D} Checking globalAuth:",!!p),p){let r=p.isAuthenticated();console.log("\u{1F50D} Initial admin status:",r),v.toggleAdminElements(r),v.updateBodyClass(r)}else console.error("\u274C globalAuth not initialized!");setTimeout(()=>{console.log("\u23F0 DOM ready, loading admin settings..."),l.loadAdminSettings()},100),console.log("\u2705 Stack initialization complete"),V();let t=document.querySelector(".article-content");t&&(new R(t),Y(),K());let i=document.querySelector(".article-list--tile");i&&new IntersectionObserver(async(c,u)=>{c.forEach(d=>{if(!d.isIntersecting)return;u.unobserve(d.target),d.target.querySelectorAll("article.has-image").forEach(async h=>{let g=h.querySelector("img"),f=g.src,y=g.getAttribute("data-key"),I=g.getAttribute("data-hash"),se=h.querySelector(".article-details"),w=await z(y,I,f);se.style.background=`
                        linear-gradient(0deg, 
                            rgba(${w.DarkMuted.rgb[0]}, ${w.DarkMuted.rgb[1]}, ${w.DarkMuted.rgb[2]}, 0.5) 0%, 
                            rgba(${w.Vibrant.rgb[0]}, ${w.Vibrant.rgb[1]}, ${w.Vibrant.rgb[2]}, 0.75) 100%)`})})}).observe(i);let s=document.querySelectorAll(".article-content div.highlight"),a="Copy",o="Copied!";s.forEach(r=>{let c=document.createElement("button");c.innerHTML=a,c.classList.add("copyCodeButton"),r.appendChild(c);let u=r.querySelector("code[data-lang]");u&&c.addEventListener("click",()=>{navigator.clipboard.writeText(u.textContent).then(()=>{c.textContent=o,setTimeout(()=>{c.textContent=a},1e3)}).catch(d=>{alert(d),console.log("Something went wrong",d)})})}),new G(document.getElementById("dark-mode-toggle")),l.registerServiceWorker()},bindAuthEvents:()=>{let n=document.getElementById("admin-login-form");n&&n.addEventListener("submit",a=>{a.preventDefault();let o=document.getElementById("admin-password");if(o&&p)if(p.authenticate(o.value))v.hideLoginModal(),console.log("Admin authenticated successfully"),setTimeout(()=>{console.log("\u{1F527} Force showing admin elements after authentication"),document.querySelectorAll("[data-admin-only]").forEach(d=>{d.style.display="block"}),document.querySelectorAll("[data-guest-only]").forEach(d=>{d.style.display="none"}),v.updateBodyClass(!0),console.log("\u2705 Admin elements forced to show")},100);else if(p.isBlocked())v.showLoginError("\u767B\u5F55\u5C1D\u8BD5\u6B21\u6570\u8FC7\u591A\uFF0C\u8BF7\u7A0D\u540E\u518D\u8BD5");else{v.showLoginError("\u5BC6\u7801\u9519\u8BEF");let c=p.getRemainingAttempts();c>0&&v.showAttemptsInfo(c)}});let e=document.getElementById("admin-modal-close"),t=document.getElementById("admin-cancel-btn");e&&e.addEventListener("click",()=>{v.hideLoginModal()}),t&&t.addEventListener("click",()=>{v.hideLoginModal()});let i=document.getElementById("admin-login-modal");i&&i.addEventListener("click",a=>{a.target===i&&v.hideLoginModal()}),document.addEventListener("keydown",a=>{a.key==="Escape"&&v.hideLoginModal()});let s=document.getElementById("admin-panel-toggle");s&&s.addEventListener("click",a=>{a.preventDefault(),l.showAdminPanel()}),document.addEventListener("click",a=>{let o=a.target;if(o){let r=o.textContent||"",c=o.parentElement?.textContent||"",u=o.closest("a")?.textContent||"";if(r.trim()==="\u7BA1\u7406\u9762\u677F"||r.includes("\u7BA1\u7406\u9762\u677F")||c.includes("\u7BA1\u7406\u9762\u677F")||u.includes("\u7BA1\u7406\u9762\u677F")||o.id==="admin-panel-toggle"||o.closest("#admin-panel-toggle")||o.classList.contains("admin-panel-trigger"))if(a.preventDefault(),a.stopPropagation(),console.log("\u{1F3AF} Admin panel click detected:",o),console.log("\u{1F3AF} Clicked text:",r),p&&p.isAuthenticated()){console.log("\u2705 User is authenticated, showing admin panel");let m=document.getElementById("admin-panel-modal");m?(console.log("\u2705 Panel found, showing directly"),m.style.display="flex",l.loadAdminSettings&&l.loadAdminSettings()):console.error("\u274C Panel not found in DOM")}else console.log("\u274C User not authenticated, cannot show admin panel")}},!0)},bindAdminPanelEvents:()=>{console.log("Binding admin panel events...");let n=document.getElementById("admin-panel-close"),e=document.getElementById("admin-panel-cancel");n&&n.addEventListener("click",()=>{l.hideAdminPanel()}),e&&e.addEventListener("click",()=>{l.hideAdminPanel()});let t=document.getElementById("admin-panel-modal");t&&t.addEventListener("click",g=>{g.target===t&&l.hideAdminPanel()}),document.querySelectorAll(".admin-tab-btn").forEach(g=>{g.addEventListener("click",f=>{let I=f.target.getAttribute("data-tab");I&&l.switchAdminTab(I)})}),setTimeout(()=>{let g=document.getElementById("admin-avatar-upload");g?(g.addEventListener("change",f=>{let y=f.target;y.files&&y.files[0]&&(console.log("\u{1F4C1} \u5934\u50CF\u6587\u4EF6\u9009\u62E9:",y.files[0].name),l.handleAvatarUpload(y.files[0]))}),console.log("\u2705 \u5934\u50CF\u4E0A\u4F20\u4E8B\u4EF6\u76D1\u542C\u5668\u5DF2\u7ED1\u5B9A")):console.warn("\u26A0\uFE0F \u5934\u50CF\u4E0A\u4F20\u5143\u7D20\u672A\u627E\u5230")},200),setTimeout(()=>{let g=document.getElementById("admin-avatar-reset");g?(g.addEventListener("click",()=>{console.log("\u{1F504} \u91CD\u7F6E\u5934\u50CF"),l.resetAvatar()}),console.log("\u2705 \u5934\u50CF\u91CD\u7F6E\u4E8B\u4EF6\u76D1\u542C\u5668\u5DF2\u7ED1\u5B9A")):console.warn("\u26A0\uFE0F \u5934\u50CF\u91CD\u7F6E\u5143\u7D20\u672A\u627E\u5230")},200);let s=document.getElementById("admin-save-settings");s&&s.addEventListener("click",()=>{l.saveAdminSettings()});let a=document.getElementById("admin-theme-color");a&&a.addEventListener("change",g=>{let f=g.target;l.updateThemeColor(f.value),localStorage.setItem("adminThemeColor",f.value)});let o=document.getElementById("admin-new-post");o&&o.addEventListener("click",()=>{l.handleNewPost()});let r=document.getElementById("admin-manage-posts");r&&r.addEventListener("click",()=>{l.handleManagePosts()});let c=document.getElementById("admin-site-stats");c&&c.addEventListener("click",()=>{l.handleSiteStats()});let u=document.getElementById("admin-dark-mode-default");u&&u.addEventListener("change",g=>{let f=g.target;l.handleDarkModeToggle(f.checked)});let d=document.getElementById("admin-change-password");d&&d.addEventListener("click",()=>{l.handlePasswordChange()});let m=document.getElementById("admin-image-manager");m&&m.addEventListener("click",()=>{l.openImageManager()});let h=document.getElementById("admin-archives-manager");h&&h.addEventListener("click",()=>{l.openArchivesManager()}),console.log("Admin panel events bound successfully")},getAuth:()=>p,showLogin:()=>{v.showLoginModal()},logout:()=>{p&&(p.logout(),document.querySelectorAll("[data-admin-only]").forEach(t=>{t.style.display="none"}),document.querySelectorAll("[data-guest-only]").forEach(t=>{t.style.display="block"}),v.updateBodyClass(!1),console.log("User logged out successfully"))},showAdminPanel:()=>{console.log("\u{1F3AF} showAdminPanel called");let n=document.getElementById("admin-panel-modal");n?(console.log("\u2705 Panel found in HTML, showing it"),n.style.display="flex",l.loadAdminSettings()):console.error("\u274C Admin panel not found in HTML! This should not happen since it is in the template.")},hideAdminPanel:()=>{let n=document.getElementById("admin-panel-modal");n&&(n.style.display="none")},switchAdminTab:n=>{document.querySelectorAll(".admin-tab-btn").forEach(i=>{i.classList.remove("active")}),document.querySelectorAll(".admin-tab-panel").forEach(i=>{i.classList.remove("active")});let e=document.querySelector(`[data-tab="${n}"]`),t=document.getElementById(`admin-tab-${n}`);e&&e.classList.add("active"),t&&t.classList.add("active")},handleAvatarUpload:async n=>{if(console.log("\u{1F4C1} \u5F00\u59CB\u5904\u7406\u5934\u50CF\u4E0A\u4F20:",n.name,n.type,n.size),!n.type.startsWith("image/")){console.error("\u274C \u6587\u4EF6\u7C7B\u578B\u9519\u8BEF:",n.type),l.showErrorMessage("\u8BF7\u9009\u62E9\u56FE\u7247\u6587\u4EF6\uFF08JPG\u3001PNG\u3001GIF\u7B49\uFF09");return}let e=5*1024*1024;if(n.size>e){console.error("\u274C \u6587\u4EF6\u8FC7\u5927:",n.size),l.showErrorMessage("\u56FE\u7247\u6587\u4EF6\u4E0D\u80FD\u8D85\u8FC75MB");return}l.showSuccessMessage("\u6B63\u5728\u4E0A\u4F20\u5934\u50CF\u5230GitHub...");try{if(typeof window<"u"&&window.githubImageUploader){console.log("\u{1F4E4} \u4F7F\u7528GitHub\u4E0A\u4F20\u5668\u4E0A\u4F20\u5934\u50CF");let t=window.githubImageUploader;t.setProgressCallback(s=>{console.log(`\u{1F4CA} \u4E0A\u4F20\u8FDB\u5EA6: ${s.progress}% - ${s.message}`)});let i=await t.uploadImage(n,{title:"\u7528\u6237\u5934\u50CF",description:"\u535A\u5BA2\u7BA1\u7406\u5458\u5934\u50CF",category:"avatars",alt:"Admin Avatar"},"avatars");if(i.success&&i.cdnUrl){console.log("\u2705 \u5934\u50CF\u4E0A\u4F20\u5230GitHub\u6210\u529F:",i.cdnUrl);let s=document.getElementById("admin-avatar-img");s&&(s.src=i.cdnUrl,console.log("\u2705 \u7BA1\u7406\u9762\u677F\u5934\u50CF\u5DF2\u66F4\u65B0")),localStorage.setItem("adminAvatar",i.cdnUrl),localStorage.setItem("adminAvatarGitHubUrl",i.url||""),localStorage.setItem("adminAvatarFileName",i.fileName||""),console.log("\u{1F4BE} \u5934\u50CFGitHub URL\u5DF2\u4FDD\u5B58\u5230localStorage"),l.updateSiteAvatar(i.cdnUrl),l.showSuccessMessage("\u5934\u50CF\u4E0A\u4F20\u6210\u529F\uFF01\u5DF2\u4FDD\u5B58\u5230GitHub"),console.log("\u2705 \u5934\u50CF\u4E0A\u4F20\u5904\u7406\u5B8C\u6210")}else throw new Error(i.error||"\u4E0A\u4F20\u5931\u8D25")}else console.log("\u26A0\uFE0F GitHub\u4E0A\u4F20\u5668\u4E0D\u53EF\u7528\uFF0C\u4F7F\u7528\u672C\u5730\u5B58\u50A8"),await l.handleAvatarUploadLocal(n)}catch(t){console.error("\u274C GitHub\u5934\u50CF\u4E0A\u4F20\u5931\u8D25:",t),l.showErrorMessage(`\u5934\u50CF\u4E0A\u4F20\u5931\u8D25: ${t.message}`),console.log("\u{1F504} \u5C1D\u8BD5\u672C\u5730\u5B58\u50A8\u4F5C\u4E3A\u5907\u9009\u65B9\u6848");try{await l.handleAvatarUploadLocal(n)}catch(i){console.error("\u274C \u672C\u5730\u5B58\u50A8\u4E5F\u5931\u8D25:",i),l.showErrorMessage("\u5934\u50CF\u4E0A\u4F20\u5B8C\u5168\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}}},handleAvatarUploadLocal:async n=>new Promise((e,t)=>{let i=new FileReader;i.onload=s=>{try{let a=s.target?.result;if(!a)throw new Error("\u56FE\u7247\u8BFB\u53D6\u5931\u8D25");console.log("\u{1F4F7} \u56FE\u7247\u8BFB\u53D6\u6210\u529F\uFF0C\u5927\u5C0F:",a.length);let o=document.getElementById("admin-avatar-img");o&&(o.src=a,console.log("\u2705 \u7BA1\u7406\u9762\u677F\u5934\u50CF\u5DF2\u66F4\u65B0"));try{localStorage.setItem("adminAvatar",a),console.log("\u{1F4BE} \u5934\u50CF\u5DF2\u4FDD\u5B58\u5230localStorage")}catch(r){console.error("\u274C localStorage\u4FDD\u5B58\u5931\u8D25:",r),t(new Error("\u5934\u50CF\u4FDD\u5B58\u5931\u8D25\uFF0C\u53EF\u80FD\u662F\u5B58\u50A8\u7A7A\u95F4\u4E0D\u8DB3"));return}l.updateSiteAvatar(a),l.showSuccessMessage("\u5934\u50CF\u4E0A\u4F20\u6210\u529F\uFF01(\u672C\u5730\u5B58\u50A8)"),console.log("\u2705 \u5934\u50CF\u672C\u5730\u4E0A\u4F20\u5904\u7406\u5B8C\u6210"),e()}catch(a){console.error("\u274C \u5934\u50CF\u5904\u7406\u5931\u8D25:",a),t(a)}},i.onerror=()=>{console.error("\u274C \u6587\u4EF6\u8BFB\u53D6\u5931\u8D25"),t(new Error("\u6587\u4EF6\u8BFB\u53D6\u5931\u8D25"))},i.readAsDataURL(n)}),resetAvatar:async()=>{console.log("\u{1F504} \u91CD\u7F6E\u5934\u50CF\u5230\u9ED8\u8BA4\u72B6\u6001");try{let n="/img/avatar_hu_f509edb42ecc0ebd.png",e=document.getElementById("admin-avatar-img");e?(e.src=n,console.log("\u2705 \u7BA1\u7406\u9762\u677F\u5934\u50CF\u5DF2\u91CD\u7F6E")):console.warn("\u26A0\uFE0F \u7BA1\u7406\u9762\u677F\u5934\u50CF\u5143\u7D20\u672A\u627E\u5230");let t=localStorage.getItem("adminAvatarFileName");if(t&&typeof window<"u"&&window.githubImageUploader)try{console.log("\u{1F5D1}\uFE0F \u5C1D\u8BD5\u4ECEGitHub\u5220\u9664\u65E7\u5934\u50CF:",t),await window.githubImageUploader.deleteImage(t)?console.log("\u2705 GitHub\u5934\u50CF\u6587\u4EF6\u5220\u9664\u6210\u529F"):console.warn("\u26A0\uFE0F GitHub\u5934\u50CF\u6587\u4EF6\u5220\u9664\u5931\u8D25\uFF0C\u4F46\u7EE7\u7EED\u91CD\u7F6E")}catch(i){console.warn("\u26A0\uFE0F GitHub\u5934\u50CF\u5220\u9664\u51FA\u9519:",i)}localStorage.removeItem("adminAvatar"),localStorage.removeItem("adminAvatarGitHubUrl"),localStorage.removeItem("adminAvatarFileName"),console.log("\u{1F5D1}\uFE0F \u5DF2\u4ECElocalStorage\u79FB\u9664\u81EA\u5B9A\u4E49\u5934\u50CF\u4FE1\u606F"),l.updateSiteAvatar(n),l.showSuccessMessage("\u5934\u50CF\u5DF2\u91CD\u7F6E\u4E3A\u9ED8\u8BA4\u5934\u50CF"),console.log("\u2705 \u5934\u50CF\u91CD\u7F6E\u5B8C\u6210")}catch(n){console.error("\u274C \u5934\u50CF\u91CD\u7F6E\u5931\u8D25:",n),l.showErrorMessage("\u5934\u50CF\u91CD\u7F6E\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}},updateSiteAvatar:n=>{console.log("\u{1F504} Updating site avatar to:",n);let e=[".site-avatar img",".site-logo",".site-avatar .site-logo","[data-avatar]",".sidebar .site-avatar img",'.sidebar img[alt*="avatar"]','.sidebar img[alt*="Avatar"]',".header-avatar img",".user-avatar img",".profile-avatar img","img.avatar","img.site-logo"],t=0;e.forEach(s=>{document.querySelectorAll(s).forEach(o=>{o&&o.tagName==="IMG"&&(o.src=n,t++,console.log(`\u2705 Updated avatar for selector: ${s}`))})}),document.querySelectorAll("img").forEach(s=>{let a=(s.alt||"").toLowerCase(),o=(s.className||"").toLowerCase(),r=(s.id||"").toLowerCase(),c=a.includes("avatar")||o.includes("avatar")||r.includes("avatar")||o.includes("site-logo")||a.includes("logo"),u=r.includes("admin")||o.includes("admin");c&&!u&&(s.src=n,t++,console.log(`\u2705 Updated additional avatar: ${s.className||s.id||s.alt||"unnamed"}`))}),console.log(`\u2705 Total avatars updated: ${t}`),setTimeout(()=>{let s=new Event("avatarUpdated");document.dispatchEvent(s)},100)},loadAdminSettings:()=>{console.log("\u{1F504} Loading admin settings...");try{let n={avatar:"/img/avatar_hu_f509edb42ecc0ebd.png",title:"lanniny-blog",description:"\u6F14\u793A\u6587\u7A3F",themeColor:"#34495e",password:"admit"};try{let e=localStorage.getItem("adminAvatar")||n.avatar,t=document.getElementById("admin-avatar-img");t&&(t.src=e,console.log("\u2705 Avatar loaded:",e!==n.avatar?"custom":"default")),e!==n.avatar&&l.updateSiteAvatar(e)}catch(e){console.warn("\u26A0\uFE0F Avatar loading failed:",e);let t=document.getElementById("admin-avatar-img");t&&(t.src=n.avatar)}try{let e=localStorage.getItem("adminSiteTitle")||n.title,t=document.getElementById("admin-site-title");if(t&&(t.value=e,console.log("\u2705 Site title loaded:",e)),e!==n.title){let i=document.querySelector(".site-name a");i&&(i.textContent=e,console.log("\u2705 Site title updated in header"))}}catch(e){console.warn("\u26A0\uFE0F Site title loading failed:",e);let t=document.getElementById("admin-site-title");t&&(t.value=n.title)}try{let e=localStorage.getItem("adminSiteDescription")||n.description,t=document.getElementById("admin-site-description");if(t&&(t.value=e,console.log("\u2705 Site description loaded:",e)),e!==n.description){let i=document.querySelector(".site-description");i&&(i.textContent=e,console.log("\u2705 Site description updated in header"))}}catch(e){console.warn("\u26A0\uFE0F Site description loading failed:",e);let t=document.getElementById("admin-site-description");t&&(t.value=n.description)}try{let e=localStorage.getItem("adminThemeColor")||n.themeColor,t=document.getElementById("admin-theme-color");t&&(t.value=e,console.log("\u2705 Theme color loaded:",e)),e!==n.themeColor&&(l.updateThemeColor(e),console.log("\u2705 Theme color applied"))}catch(e){console.warn("\u26A0\uFE0F Theme color loading failed:",e);let t=document.getElementById("admin-theme-color");t&&(t.value=n.themeColor)}try{let e=localStorage.getItem("adminPassword");e&&p?p.config?(p.config.adminPassword=e,console.log("\u2705 Admin password loaded from localStorage")):console.warn("\u26A0\uFE0F globalAuth.config not available, password not loaded"):console.log("\u2139\uFE0F No saved password found, using default")}catch(e){console.warn("\u26A0\uFE0F Admin password loading failed:",e)}console.log("\u2705 Admin settings loading completed")}catch(n){console.error("\u274C Critical error in loadAdminSettings:",n),console.log("\u{1F527} Attempting to recover with default values...")}},saveAdminSettings:()=>{console.log("\u{1F4BE} Saving admin settings...");let n=document.getElementById("admin-save-settings"),e=n?.textContent||"\u4FDD\u5B58\u8BBE\u7F6E";try{n&&(n.disabled=!0,n.innerHTML=`
                    <svg class="admin-icon admin-loading" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M21 12a9 9 0 11-6.219-8.56"></path>
                    </svg>
                    \u4FDD\u5B58\u4E2D...
                `,console.log("\u{1F504} Save button set to loading state"));let t=0,i=0;if(!l.FormValidator.validateAllFields()){n&&(n.disabled=!1,n.innerHTML=`
                        <svg class="admin-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z"></path>
                            <polyline points="17,21 17,13 7,13 7,21"></polyline>
                            <polyline points="7,3 7,8 15,8"></polyline>
                        ...
                        ${e}
                    `),l.showErrorMessage("\u8BF7\u4FEE\u6B63\u8868\u5355\u4E2D\u7684\u9519\u8BEF\u540E\u518D\u4FDD\u5B58");return}let a=document.getElementById("admin-site-title");if(a){i++;let c=l.FormValidator.validateTitle(a.value);if(c.isValid){let u=a.value.trim();localStorage.setItem("adminSiteTitle",u);let d=document.querySelector(".site-name a");d&&(d.textContent=u,console.log("\u2705 Site title saved and updated:",u)),t++}else console.warn("\u26A0\uFE0F Site title validation failed:",c.message)}let o=document.getElementById("admin-site-description");if(o){i++;let c=l.FormValidator.validateDescription(o.value);if(c.isValid){let u=o.value.trim();localStorage.setItem("adminSiteDescription",u);let d=document.querySelector(".site-description");d&&(d.textContent=u,console.log("\u2705 Site description saved and updated:",u)),t++}else console.warn("\u26A0\uFE0F Site description validation failed:",c.message)}let r=document.getElementById("admin-theme-color");if(r){i++;let c=l.FormValidator.validateThemeColor(r.value);if(c.isValid){let u=r.value;localStorage.setItem("adminThemeColor",u),l.updateThemeColor(u),console.log("\u2705 Theme color saved and applied:",u),t++}else console.warn("\u26A0\uFE0F Theme color validation failed:",c.message)}setTimeout(()=>{n&&(n.disabled=!1,n.innerHTML=`
                        <svg class="admin-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z"></path>
                            <polyline points="17,21 17,13 7,13 7,21"></polyline>
                            <polyline points="7,3 7,8 15,8"></polyline>
                        ...
                        ${e}
                    `),t===i&&i>0?(l.showSuccessMessage(`\u8BBE\u7F6E\u5DF2\u4FDD\u5B58\uFF01(${t}/${i}\u9879)`),console.log(`\u2705 All settings saved successfully (${t}/${i})`),l.hideAdminPanel()):t>0?(l.showSuccessMessage(`\u90E8\u5206\u8BBE\u7F6E\u5DF2\u4FDD\u5B58 (${t}/${i}\u9879)`),console.log(`\u26A0\uFE0F Partial save completed (${t}/${i})`)):(l.showErrorMessage("\u6CA1\u6709\u6709\u6548\u7684\u8BBE\u7F6E\u9700\u8981\u4FDD\u5B58"),console.log("\u274C No valid settings to save"))},800)}catch(t){console.error("\u274C Error saving admin settings:",t),n&&(n.disabled=!1,n.innerHTML=`
                    <svg class="admin-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z"></path>
                        <polyline points="17,21 17,13 7,13 7,21"></polyline>
                        <polyline points="7,3 7,8 15,8"></polyline>
                ...
                <span>${e.message}</span>
            `,t.classList.add(e.isValid?"validation-success":"validation-error"),n.includes("password")&&e.strength){let a=`strength-${e.strength}`;s.classList.add(a)}t.parentElement?.appendChild(s)},validateAllFields:()=>{let n=!0,e=document.getElementById("admin-site-title");if(e){let s=l.FormValidator.validateTitle(e.value);l.FormValidator.showFieldValidation("admin-site-title",s),s.isValid||(n=!1)}let t=document.getElementById("admin-site-description");if(t){let s=l.FormValidator.validateDescription(t.value);l.FormValidator.showFieldValidation("admin-site-description",s),s.isValid||(n=!1)}let i=document.getElementById("admin-theme-color");if(i){let s=l.FormValidator.validateThemeColor(i.value);l.FormValidator.showFieldValidation("admin-theme-color",s),s.isValid||(n=!1)}return n}},setupFormValidation:()=>{console.log("\u{1F527} Setting up real-time form validation...");let n=document.getElementById("admin-site-title");n&&(n.addEventListener("input",()=>{let s=l.FormValidator.validateTitle(n.value);l.FormValidator.showFieldValidation("admin-site-title",s)}),n.addEventListener("blur",()=>{let s=l.FormValidator.validateTitle(n.value);l.FormValidator.showFieldValidation("admin-site-title",s)}));let e=document.getElementById("admin-site-description");e&&(e.addEventListener("input",()=>{let s=l.FormValidator.validateDescription(e.value);l.FormValidator.showFieldValidation("admin-site-description",s)}),e.addEventListener("blur",()=>{let s=l.FormValidator.validateDescription(e.value);l.FormValidator.showFieldValidation("admin-site-description",s)}));let t=document.getElementById("admin-new-password");t&&(t.addEventListener("input",()=>{if(t.value.length>0){let s=l.FormValidator.validatePassword(t.value);l.FormValidator.showFieldValidation("admin-new-password",s)}}),t.addEventListener("blur",()=>{if(t.value.length>0){let s=l.FormValidator.validatePassword(t.value);l.FormValidator.showFieldValidation("admin-new-password",s)}}));let i=document.getElementById("admin-theme-color");i&&i.addEventListener("change",()=>{let s=l.FormValidator.validateThemeColor(i.value);l.FormValidator.showFieldValidation("admin-theme-color",s)}),console.log("\u2705 Real-time form validation setup complete")},updateThemeColor:n=>{document.documentElement.style.setProperty("--accent-color",n);let e=document.querySelector(".admin-color-preview");e&&(e.style.backgroundColor=n)},changeAdminPassword:()=>{let n=document.getElementById("admin-new-password");if(!n){l.showErrorMessage("\u5BC6\u7801\u8F93\u5165\u6846\u672A\u627E\u5230");return}let e=n.value.trim(),t=l.FormValidator.validatePassword(e);if(!t.isValid){l.showErrorMessage(t.message),l.FormValidator.showFieldValidation("admin-new-password",t);return}if(!(t.strength==="weak"&&!confirm("\u60A8\u7684\u5BC6\u7801\u5F3A\u5EA6\u8F83\u5F31\uFF0C\u5EFA\u8BAE\u4F7F\u7528\u5305\u542B\u5927\u5C0F\u5199\u5B57\u6BCD\u3001\u6570\u5B57\u548C\u7279\u6B8A\u5B57\u7B26\u7684\u5BC6\u7801\u3002\u662F\u5426\u7EE7\u7EED\uFF1F")))try{p&&(p.config&&(p.config.adminPassword=e,console.log("\u2705 Updated globalAuth.config.adminPassword")),typeof p.updatePassword=="function"&&(p.updatePassword(e),console.log("\u2705 Called globalAuth.updatePassword()"))),localStorage.setItem("adminPassword",e),console.log("\u2705 Saved new password to localStorage"),n.value="";let i=n.parentElement?.querySelector(".validation-message");i&&i.remove(),n.classList.remove("validation-success","validation-error"),l.showSuccessMessage(`\u5BC6\u7801\u5DF2\u6210\u529F\u66F4\u65B0\uFF01\u5BC6\u7801\u5F3A\u5EA6: ${t.strength==="strong"?"\u5F3A":t.strength==="medium"?"\u4E2D\u7B49":"\u5F31"}`),console.log("\u{1F510} Password change completed successfully")}catch(i){console.error("\u274C Password change failed:",i),l.showErrorMessage("\u5BC6\u7801\u66F4\u65B0\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}},openImageManager:()=>{console.log("\u{1F5BC}\uFE0F Opening image manager..."),window.open("/page/image-manager/","_blank"),l.hideAdminPanel(),console.log("\u2705 Image manager opened in new tab")},openArchivesManager:()=>{console.log("\u{1F4DA} Opening archives manager..."),window.open("/archives/","_blank"),l.hideAdminPanel(),console.log("\u2705 Archives manager opened in new tab")},showSuccessMessage:n=>{l.showNotification(n,"success")},showErrorMessage:n=>{l.showNotification(n,"error")},showNotification:(n,e="success")=>{let t=document.createElement("div");t.className=`admin-notification admin-notification-${e}`,t.innerHTML=`
            <div class="admin-notification-content">
                <svg class="admin-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        `,showAdminPanel:()=>{let n=document.getElementById("admin-panel-modal");n&&(n.style.display="flex",l.loadAdminSettings(),setTimeout(()=>{l.setupFormValidation()},100))},hideAdminPanel:()=>{let n=document.getElementById("admin-panel-modal");n&&(n.style.display="none")},switchAdminTab:n=>{document.querySelectorAll(".admin-tab-btn").forEach(i=>{i.classList.remove("active")}),document.querySelectorAll(".admin-tab-panel").forEach(i=>{i.classList.remove("active")});let e=document.querySelector(`[data-tab="${n}"]`),t=document.getElementById(`admin-tab-${n}`);e&&e.classList.add("active"),t&&t.classList.add("active")},updateThemeColor:n=>{document.documentElement.style.setProperty("--accent-color",n);let e=document.querySelector(".admin-color-preview");e&&(e.style.backgroundColor=n)},handleNewPost:()=>{console.log("\u{1F4DD} Creating new post..."),l.showSuccessMessage("\u65B0\u5EFA\u6587\u7AE0\u529F\u80FD\u6B63\u5728\u5F00\u53D1\u4E2D\uFF0C\u656C\u8BF7\u671F\u5F85\uFF01")},handleManagePosts:()=>{console.log("\u{1F4CB} Opening article manager..."),typeof window.articleManager<"u"?(window.articleManager.openManager(),console.log("\u2705 Article manager opened")):(console.log("\u26A0\uFE0F Article manager not found, attempting to initialize..."),Promise.resolve().then(()=>(D(),Z)).then(({ArticleManager:n})=>{let e=new n;window.articleManager=e,e.openManager(),console.log("\u2705 Article manager initialized and opened")}).catch(n=>{console.error("\u274C Failed to initialize article manager:",n),l.showErrorMessage("\u6587\u7AE0\u7BA1\u7406\u5668\u521D\u59CB\u5316\u5931\u8D25\uFF0C\u8BF7\u5237\u65B0\u9875\u9762\u91CD\u8BD5")}))},handleSiteStats:()=>{console.log("\u{1F4CA} Showing site statistics...");let n={totalPosts:5,totalViews:1234,totalComments:56,lastUpdate:new Date().toLocaleDateString()},e=`
            \u{1F4CA} \u7AD9\u70B9\u7EDF\u8BA1\u4FE1\u606F\uFF1A
            \u2022 \u6587\u7AE0\u603B\u6570\uFF1A${n.totalPosts} \u7BC7
            \u2022 \u603B\u8BBF\u95EE\u91CF\uFF1A${n.totalViews} \u6B21
            \u2022 \u8BC4\u8BBA\u603B\u6570\uFF1A${n.totalComments} \u6761
            \u2022 \u6700\u540E\u66F4\u65B0\uFF1A${n.lastUpdate}
        `;l.showSuccessMessage(e)},handlePasswordChange:()=>{let n=document.getElementById("admin-new-password");if(!n||!n.value.trim()){l.showErrorMessage("\u8BF7\u8F93\u5165\u65B0\u5BC6\u7801");return}let e=n.value.trim();if(e.length<4){l.showErrorMessage("\u5BC6\u7801\u957F\u5EA6\u81F3\u5C114\u4E2A\u5B57\u7B26");return}p&&p.config?(p.config.adminPassword=e,localStorage.setItem("adminPassword",e),n.value="",l.showSuccessMessage("\u7BA1\u7406\u5458\u5BC6\u7801\u5DF2\u66F4\u65B0"),console.log("\u2705 Admin password updated")):l.showErrorMessage("\u5BC6\u7801\u66F4\u65B0\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")},handleDarkModeToggle:n=>{console.log("\u{1F319} Dark mode toggle:",n),localStorage.setItem("adminDarkModeDefault",n.toString()),n?(document.documentElement.setAttribute("data-scheme","dark"),l.showSuccessMessage("\u5DF2\u8BBE\u7F6E\u9ED8\u8BA4\u6DF1\u8272\u6A21\u5F0F")):(document.documentElement.setAttribute("data-scheme","light"),l.showSuccessMessage("\u5DF2\u8BBE\u7F6E\u9ED8\u8BA4\u6D45\u8272\u6A21\u5F0F"))}};window.addEventListener("load",()=>{setTimeout(function(){l.init()},0)});window.Stack=l;window.createElement=N;window.StackAuth=p;document.addEventListener("DOMContentLoaded",()=>{console.log("\u{1F3A8} Modern blog layout initialized"),document.querySelectorAll('a[href^="#"]').forEach(n=>{n.addEventListener("click",function(e){e.preventDefault();let t=document.querySelector(this.getAttribute("href"));t&&t.scrollIntoView({behavior:"smooth",block:"start"})})}),document.querySelectorAll(".btn").forEach(n=>{n.addEventListener("click",function(){this.classList.contains("btn-loading")||(this.classList.add("btn-loading"),setTimeout(()=>{this.classList.remove("btn-loading")},2e3))})}),document.querySelectorAll(".article-card, .article-list article").forEach(n=>{n.addEventListener("mouseenter",function(){this.style.transform="translateY(-4px)"}),n.addEventListener("mouseleave",function(){this.style.transform="translateY(0)"})}),console.log("\u2705 Modern layout enhancements applied")});l.registerServiceWorker=()=>{"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").then(n=>{console.log("\u2705 Service Worker \u6CE8\u518C\u6210\u529F:",n.scope),n.addEventListener("updatefound",()=>{let e=n.installing;e&&e.addEventListener("statechange",()=>{e.state==="installed"&&navigator.serviceWorker.controller&&l.showUpdateNotification()})})}).catch(n=>{console.log("\u274C Service Worker \u6CE8\u518C\u5931\u8D25:",n)})})};l.showUpdateNotification=()=>{let n=document.createElement("div");n.style.cssText=`
        position: fixed;
        top: 20px;
        right: 20px;
        background: var(--accent-color);
